...
1. Length
    - 存储器rd_req TLP并不包含Payload( PS:Payload和length不同)
    - Length 字段的最小单位为DW
    - 单位DW, 当该字段为n时,表Mem_rd_req示需要获得的数据长度或者当前报文的数据长度为n个DW
        - 其中0≤n≤0x3FF（ps：4K内）
        - 当n等于0时,表示数据长度为1024个DW
    - 不同报文中length含义:
        - req_rd：从目标读取的数据长度；
...
2. DW BE字段
    - TH字段==0(基础功能,常规模式)
        - PCIe以字节为单位进行传递, 但Length字段以DW为最小单位
        - 为此，TLP使用LBE和FBE这两个字段进行字节使能,实现有效数据以字节为单位
        - “Zero⁃Length”读请求
            - Length长度为1DW,而FBE 字段和LBE字段都为0b0000,即所有字节都不使能
        - 规则:
            - 1st DW BE [3:0]指明所请求的第一个(Length=1时为唯一一个)DW的Byte Enable
                - 如果req的Length指示的长度大于1DW,则此字段不得等于0000b。
            - LBE[3:0]字段指明请求的最后一个DW的Byte Enable
                - 如果req的Length指示的长度等于1DW,则此字段必须等于0000b。
                - 如果req的Length指示的长度大于1DW,则此字段不得等于0000b。
            - 对于Byte Enable的每个比特:
                - 值为0b表示:不得写入相应的数据字节,或者如果non-prefetchable,则不得读取。
                - 值为1b表示:必须在写入或读取相应的数据字节。
            - 对于长度为1DW的所有请求类型,1st DW BE [3:0]字段中允许使用非连续的 Byte Enable
                - 非连续的Byte Enable指的是1010b,0101b,1001b,1011b,1101b 这种形式
            - 对于长度为2DW的QW对齐的Mem_req,1st DW BE [3:0]和 LBE [3:0]中均允许非连续的Byte Enable。
            - 长度为2DW的所有非QW 对齐的Mem_req和长度为3DW或更大的Mem_req必须仅启用与请求的第一个DW和最后一个DW之间的数据连续的字节
                - 连续的Byte Enable的例子:
                    - 1st DW BE: 1100b,LBE: 0011b
                    - 1st DW BE: 1000b,LBE: 0111b
            - Byte Enable 字段的比特分布,它们在 req header 中的位置以及所引用数据的相应字节之间的对应关
                - ![](./99_img/Snipaste_2025-07-03_16-25-46.png)
            - 允许长度为1 DW 且未启用任何字节的 Write req,除非特别说明,否者这对Completer 没有影响
    ...
    - TH字段==1(变形功能，支持Processing Hints特性的包)
        ...
        - 对于TH字段置1时，Mem_rd_req,Byte Enable 隐含以下值
            - 请求的Length为1,则FBE的值隐含为1111b,LBE的值隐含为0000b -->只有1个DW，且4个字节都有效，因为只有1个DW，所以LBE都是0
            - 请求的Length请求的数据大于1,FBE和LBE的值隐含为1111b --> 天然DW对齐
        - 带有TH位设置且针对非可预取内存空间的存储器rd_req,仅当能够保证此类读取操作的完成不会产生不良副作用时方可发起。
            - 对于某些基址寄存器(BAR),即使其映射的某些存储位置具有读取副作用,仍可能设置了可预取位,相关注意事项请参阅第7.5.1.2.1节。

#### 2. cpl
1. ![](./99_img/Snipaste_2025-07-03_14-45-46.png)
2. BCM(Byte Count Modified)字段
    - 由PCI⁃X设备设置。 
    - 而PCIe 设备不能操纵BCM位和
3. Byte Count
...
4. Lower Address 字段
    ...
    - 值并不存在FBE和LBE字段,因此接收端必须使用存储器读完成TLP的Low Address字段,识别一个TLP 中包含数据的起始地址。
    - 存储器读完成TLP中Data0(即第一个双字)中包含的有效数据, 以及Data( Length⁃1)(即最后一个双字) 中包含的有效数据
    - 存储器读完成TLP并不含有FBE字段, 此时Capric卡需要使用存储器读完成TLP中的Lower Address 字段识别 Data0 中的有效字节
    ...
    - 存储器读完成TLP并没有设置LBE字段,Capric卡需要使用Byte Count和 LowerAddress字段联合识别Data(Length⁃1)中的有效数据。
